/**
 * LBS Scroll 左右滚动  复制删除节点方式 图片延迟加载
 * Date: 2012-5-18
 * ===================================================
...
;(function(window, document) {
	...
	var Scroll = function(opts) {
		...
		this.num = Math.ceil(this.wrapper.clientWidth / this.width); //显示区域能显示几个
	...
	Scroll.prototype = {
		...
		_initSet: function() {
			...
			if (this.lazyLoad) {
				for (i = 0; i < this.num; i++) {
					var el = this.scroller.children[i];
					this._loadIMG(el);
				}
			}
		},
		...
		scrollLeft: function() {
			...
			if (this.lazyLoad) {
				//向左 图片显示区域个数 到 图片显示区域个数加滚动个数
				for (i = this.num; i < this.num + this.count; i++) {
					var el = this.scroller.children[i];
					this._loadIMG(el);
				}
			}
		...
		scrollRight: function() {
			...
			for (; i < this.count; i++) {
				...
				//向右边 比较方便
				if (this.lazyLoad) this._loadIMG(itemClone);
			}
