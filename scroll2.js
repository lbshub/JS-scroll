...
;(function(window, document) {
	...
	var Scroll = function(opts) {
		...
		this.amount =  Math.floor(this.wrapper.offsetWidth / this.width); //显示区域能显示几个
		...
		this.totalWidth = this.width * this.length; //总宽度
		this.num = Math.ceil(this.totalWidth / this.distance); //大约滚几次才能滚完所有元素 有几屏
		this.index = 0;
	...
	Scroll.prototype = {
		...
		_scroll: function() {
			var _this = this;
			this.amimated = true;
			this._animate(this.scroller, 'left', -this.index * this.distance, function() {
		...
		scrollRight: function() {
			...
			if (this.index === 0) {
				this.index = this.num;
				this.scroller.style.left = -this.num * this.distance + 'px';
			}
			--this.index;
			this._scroll();
		},
